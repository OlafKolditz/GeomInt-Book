...
Direct Numerical Simulations (DNS) of effective physical properties of single-phase flow through porous or fractured solid materials can be performed directly on the pore scale of the porous soil or rock. Morphological information, the basis for the subsequent DNS, is obtained as segmented (binarized) voxel-data from $\mu$ X-Ray Computed Tomography (XRCT). In general, numerical simulations of flow processes on XRCT-data at small to moderate Reynolds (Re) numbers could be performed by mesh-based Finite Element, Finite Differences or Finite Volume methods \todo{HS: References}.
...
it became attractive for various problems fluid dynamics like single and multi-phase fluid mechanics with internal interfaces \todo{Rakulan's work}, suspension flow, \todo{Nadine's work} and single and multi-phase flow in porous media \todo{Rakulan's work}.
...
\todo{HS: Bitte hier noch Referenzen, Übersichtsartikel Monaghan, Rakulan etc.}
...
We adopted the notation used in mixture theory \todo{HS: Steet Renner 2019} where a subscript is used for kinematical quantities and a superscript elsewhere.
...
\todo{HS: Koennen wir hier nochmals auf ein review paper zur SPH und Navier-Stokes verlinken?}